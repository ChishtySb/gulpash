    ...
    <div className="space-y-6 font-sans">
      ...
      {subview === 'hero' && (
        <div className="bg-white p-6 border border-stone-200 rounded-lg space-y-6 shadow-xs">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono font-bold bg-amber-100 text-amber-900 px-2 py-0.5 rounded uppercase">
                EXACT MEDIA SPECS ENFORCED
              </span>
            </div>
            <h3 className="font-serif text-lg font-bold text-stone-900 mt-1">
              Homepage Hero Slides & Runway Video
            </h3>
            <p className="text-xs text-stone-500">
              Configure the primary visual introduction to GulPash. Both high-res imagery and runway video loops are supported with dedicated desktop and mobile viewports.
            </p>
          </div>

          {/* Media Mode Switcher (Image vs Video) */}
          <div className="flex items-center gap-3 p-3 bg-stone-50 rounded-lg border border-stone-200 text-xs">
            <span className="font-bold text-stone-800">Hero Media Presentation:</span>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setCmsConfig({
                  ...cmsConfig,
                  hero: { ...cmsConfig.hero, type: 'image' }
                })}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded font-medium cursor-pointer transition-colors ${
                  (cmsConfig.hero?.type || 'image') === 'image'
                    ? 'bg-stone-900 text-white'
                    : 'bg-white border border-stone-300 text-stone-700'
                }`}
              >
                <ImageIcon className="w-3.5 h-3.5" />
                <span>Option A: Studio Photography (Image)</span>
              </button>
              ...
              <button
                ...
                onClick={() => setCmsConfig({
                  ...cmsConfig,
                  hero: { ...cmsConfig.hero, type: 'video' }
                })}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded font-medium cursor-pointer transition-colors ${
              ...
              >
                ...
                <span>Option B: Runway Video Loop (Video)</span>
              </button>
            </div>
          </div>

          {/* Media Uploaders with Clear Specs Guidance */}
          ...
          {/* Headings & CTA Controls */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs pt-4 border-t border-stone-100">
            <div className="space-y-1">
              <label className="block font-bold text-stone-800">Hero Main Headline</label>
              <input
                type="text"
            ...
            <div className="space-y-1">
              <label className="block font-bold text-stone-800">Hero Subtitle / Atelier Tagline</label>
              <input
                type="text"
            ...
            <div className="space-y-1">
              <label className="block font-bold text-stone-800">Primary Button Label</label>
              <input
                type="text"
                value={cmsConfig.hero?.buttonText || 'Explore Collection'}
                onChange={(e) => setCmsConfig({
                  ...cmsConfig,
                  hero: { ...cmsConfig.hero, buttonText: e.target.value }
                })}
            ...
            <div className="space-y-1">
              <label className="block font-bold text-stone-800">Button Link Target</label>
              <input
                type="text"
                value={cmsConfig.hero?.linkUrl || '/shop'}
                onChange={(e) => setCmsConfig({
                  ...cmsConfig,
                  hero: { ...cmsConfig.hero, linkUrl: e.target.value }
                })}
          ...
          <div className="flex justify-end pt-4 border-t border-stone-100">
            <button
              type="button"
              onClick={() => handleSaveCMS('Hero Presentation')}
              disabled={isSaving}
              className="flex items-center gap-2 bg-stone-900 hover:bg-stone-800 text-white px-6 py-2.5 rounded text-xs font-medium uppercase tracking-wider transition-colors cursor-pointer"
            >
              <Save className="w-3.5 h-3.5" />
              <span>{isSaving ? 'Saving...' : 'Save Hero Presentation'}</span>
            </button>
          </div>
        </div>
      )}
