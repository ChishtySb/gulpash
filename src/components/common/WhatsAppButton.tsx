...
export const WhatsAppButton: React.FC = () => {
  ...
  // If WhatsApp assistance is disabled globally or floating button is toggled off
  if (!waConfig.enabled || !waConfig.showFloatingButton) {
    return null;
  }

  const whatsappUrl = getWhatsAppUrl(waConfig.destinationNumber, waConfig.defaultMessage);
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-40 flex items-end gap-3 font-sans">
      ...
      {showTooltip && (
        <div className="hidden sm:flex items-center gap-2 bg-white text-[#222] text-xs py-2 px-3 rounded-md shadow-xl border border-[#e8e3dc] max-w-xs animate-in fade-in slide-in-from-bottom-2">
          ...
          <button 
            ...
            className="text-[#999] hover:text-black p-0.5 cursor-pointer"
      ...
      <a
        ...
        aria-label={waConfig.displayLabel || "Chat on WhatsApp"}
        title={`${waConfig.displayLabel || 'Chat on WhatsApp'} (${waConfig.number})`}
      >
